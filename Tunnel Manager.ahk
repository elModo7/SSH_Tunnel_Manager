...
; Last changed date: 16/01/2024 18:56
...
SERIAL:
ELMOD-O7777-77777-77777 <- Master key serial
MONTE-SSORI-2023C-YBERS <- Master key serial
...
FUTURE PLANS:
- Track Updates with GitHub Releases (https://api.github.com/repos/elModo7/Tunnel_Manager/releases/latest)
*/
#NoEnv
...
global serial := readSerial()
global validationServerIp := "listener.zapto.org"
global validationServerPort := 25564
global りせんしあヴアリダダ = 0 ; Validated License Yes/No
...
if(notray){
	...
	Menu, Tray, Add, Unbind License, ResetLicenseInfo
	Menu, Tray, Disable, Unbind License
	Menu, Tray, Add
...
; Pass a serial into this function, it will encrypt it and insert it into the registry
writeSerial(unhandledParam := ""){
	global
	serial := neutron.doc.getElementById("serial1").value "-" neutron.doc.getElementById("serial2").value "-" neutron.doc.getElementById("serial3").value "-" neutron.doc.getElementById("serial4").value
	StringUpper, serial, serial
	RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial, % Crypt.Encrypt.String("AES", "ECB", serial, "elModo7Soft-2023")
	gosub, createPCData
	checkValidSerial()
}

; Gets the serial written in the registry and decrypts it
readSerial(unhandledParam := ""){
	global
	RegRead, serial, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial
	serial := serial != "" ? Crypt.Decrypt.String("AES", "ECB", serial, "elModo7Soft-2023") : ""
	return serial
}

checkValidSerial(unhandledParam := ""){
	global
	; master key will not check for online verification (allows offline usage too)
	; there is a crack included for my personal use and for the sake of Montessori's 2023 IT Cybersecurity Master repo
	if(serial != "ELMOD-O7777-77777-77777" && serial != "MONTE-SSORI-2023C-YBERS"){
		if(serial == "" || serial = "---"){
			; no serial, ask for it
			neutron.wnd.askForSerial()
		}else{
			; Query saved serial on the server
			gosub, createPCData
			myTcp := new SocketTCP()
			myTcp.connect(validationServerIp, validationServerPort)
			myTcp.onRecv := Func("OnTcpRecvCliente")
			;~ myTcp.sendText(Crypt.Encrypt.String("AES", "ECB", serial "`n" InfoPC, "TunnelManagerEM7"))
			myTcp.sendText(AES.Encrypt(serial "`n" InfoPC, "TunnelManagerEM7", 256))
			Settimer, checkValidation, 3000 ; If after 3 seconds the key is not validated, the program will close
		}
	}else{
		neutron.wnd.noAskForSerial()
	}
}
...
ResetLicenseInfo:
	MensajeEnviar := "!UnbindSerial:"serial
	myTcp := new SocketTCP()
	myTcp.connect(validationServerIp, validationServerPort)
	myTcp.onRecv := Func("OnTcpRecvCliente")
	myTcp.sendText(AES.Encrypt(MensajeEnviar, "TunnelManagerEM7", 256))
	TrayTip, Unbound Serial, Now you can install Tunnel Manager on other computer!
	RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial, % ""
	Sleep, 5000
	ExitApp
return
...
checkValidation:
	if(りせんしあヴアリダダ = 0)
	{
		TrayTip, Error, Could not verify this license key.`nValidation server may be down?
		Sleep, 5000
		ExitApp
	}
	else if(りせんしあヴアリダダ = 1)
	{
		SetTimer, checkValidation, Off
	}
return

createPCData:
	global InfoPC = % RunCMD( "cmd /c set | findstr COMPUTERNAME & set | findstr NUMBER_OF_PROCESSORS & set | findstr PROCESSOR_IDENTIFIER & set | findstr PROCESSOR_LEVEL & set | findstr PROCESSOR_REVISION" )
return

OnTcpRecvCliente(this)
{
	global
	MSG_Recv := this.RecvText()
	MSG_RecvDecoded := AES.Decrypt(MSG_Recv, "TunnelManagerEM7", 256)
	if(SubStr(MSG_RecvDecoded,1,21) = "らりせんしあハシドゔありだだコレクためンテ") ; License validated successfully (in Japanese)
	{
		DatoRemoto := SubStr(MSG_RecvDecoded,22,StrLen(MSG_RecvDecoded))
		DatoLocal = % RunCMD( "cmd /c set | findstr PROCESSOR_REVISION" )
		DatoLocal := SubStr(DatoLocal,20,StrLen(DatoLocal))
		StringReplace, DatoLocal,DatoLocal, `r,, All
		StringReplace, DatoLocal,DatoLocal, `n,, All
		if(DatoLocal = DatoRemoto)
		{
			りせんしあヴアリダダ = 1
			neutron.wnd.noAskForSerial()
			if(!silent){
				TrayTip, License Validated, This license was validated succesfully.
			}
		}
	}
	else if (MSG_Recv == "Error: Invalid Serial")
	{
		TrayTip, Error, This license key is not a valid license key.
		RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial, % ""
		Sleep, 3000
		ExitApp
	}
	else if (MSG_Recv == "Error: This license is only valid for one PC, deactivate it first to use it in this one.")
	{
		TrayTip, Error, This license is only valid for one PC`, deactivate it first to use it in this one.
		RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial, % ""
		Sleep, 3000
		ExitApp
	}
...
plinkPromptDownload(){
	...
	if(!FileExist(A_Temp "\proxy.exe")){
		; Assuming it's the first run, it writes down a Master Serial to registry since I'm not doing further online licensing on this software and I'm open sourcing it
		RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\elModo7's Tunnel Manager, Serial, fuXpSg1ANzUZKEJlbj0cu2tH3urqCj0NtFK6/0TGDeg=
...
downloadLatestVersion(unhandledParam := ""){
	Run, https://elmodo7.github.io/tunnel_manager/
}
